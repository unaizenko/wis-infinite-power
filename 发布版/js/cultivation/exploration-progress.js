(function defineExplorationProgress(WIS) {
  ...
  function project(words){if(!Array.isArray(words))return B.BN(words);return normalize(words).reduceRight((sum,word)=>{if(!/^-?\d/.test(word))return B.add(sum,signed().project(word));const t=parse(word),digits=abs(t.c).toString();
  ...
  const F=()=>WIS.Core.FiniteProgress;
  const fresh=()=>({version:2,natural:F().create(10),seize:"0",levelResidual:[]});
  ...
  function ensure(s) {
    if(s.explorationRewards?.version===2)return s.explorationRewards;
    const p=legacyEnsure(s),natural=project(p.natural),seize=project(p.seize);
    if(natural.lt(0)||seize.lt(0))fail('进度为负');
    s.explorationRewards={version:2,natural:F().create(demandAt(s),natural),
      seize:String(seize),levelResidual:p.levelResidual||[],
      ...(p.approximation?{approximation:{code:p.approximation.code,exactRemainder:false}}:{})};
    return s.explorationRewards;
  }
  function validate(s) {
    const p=ensure(s);F().validate(p.natural);
    if(!B.BN(p.seize).isFinite()||B.lt(p.seize,0))fail('夺基进度无效');
  ...
  function layeredNatural(s,incoming,ceiling,previous){
    const progress=add(previous.natural,incoming),current=levelWords(s);
    ...
    if(!B.gt(target,n)){installNatural(s,previous,progress);return B.ZERO;}
    const words=[...signed().expand(progress)];
    const low=words.filter(w=>!String(w).startsWith('-')&&signed().project(w).abs().layer<2);
    // Preserve represented low words before projecting into the finite point.
    // The existing high geometric inverse and cap debit remain unchanged.
    const atCap=compare([target],ceiling)>=0;
    const main=atCap?signed().value(ceiling):target;
    s.naturalTreasureLevel=main;
    installNatural(s,previous,atCap?normalize([B.max(0,B.sub(pv,B.max(0,B.sub(layeredCost(main),cv))))]):normalize(low),
      atCap?minus(ceiling,[main]):[],{code:'high-geometric-batch',exactRemainder:false});
    WIS.Core.Effects?.invalidate?.();return B.sub(main,n);
  }
  function settleNatural(s,amount,frozen,previous){const ceiling=frozen?.cap || capWords(s);
    ...
    if(current===null||[...signed().expand(incoming),...signed().expand(previous.natural)].some(w=>
      ...
      return layeredNatural(s,incoming,ceiling,previous);
    const priorProgress=previous.natural,progress=add(priorProgress,incoming),credit=add(cumulative(current),progress);
    if(compare(credit,cumulative(current+1n))<0){installNatural(s,previous,progress);return B.ZERO;}
    let target;
    ...
    // A closed cap defers awards; it never discards the represented surplus.
    s.naturalTreasureLevel=main;
    installNatural(s,previous,remainder,L().subtract(words,[main]));
    if(gained>0n)WIS.Core.Effects?.invalidate?.();return L().project(String(gained));
  }
  // Cumulative/inverse arithmetic is local to a boundary calculation. The
  // state stays finite v2 throughout; no v2 -> legacy state -> v2 transition.
  function installNatural(s,previous,words,levelResidual=previous.levelResidual,approximation=previous.approximation) {
    const base={version:2,natural:ensure(s).natural,seize:previous.seize,levelResidual:levelResidual||[],
      ...(approximation?{approximation:{code:approximation.code,exactRemainder:false}}:{})};
    s.explorationRewards=base;
    s.explorationRewards={...base,natural:F().create(demandAt(s),project(words))};
  }
  function natural(s,amount,frozen) {
    const p=ensure(s),ceiling=frozen?.cap||capWords(s);
    if(!(frozen?.eligible??s.goldenCoreUnlocked))return B.ZERO;
    const gain=Array.isArray(amount)?project(amount):B.BN(amount);
    const next=F().bulk(p.natural,gain,{level:signed().value(levelWords(s)),cap:signed().value(ceiling),settle(){
      const point=p.natural,local={...p,natural:normalize([point.requirement,B.BN(point.remaining).neg(),point.carry])};
      return {settled:true,gained:settleNatural(s,gain,frozen,local)};
    }});
    if(next.settled)return next.gained;
    s.explorationRewards={...p,natural:next.point};return B.ZERO;
  }
  function seize(s,amount) {
    if(s.unlockedAchievements?.seizeFoundation)return false;
    const p=ensure(s),gain=Array.isArray(amount)?project(amount):B.BN(amount);
    if(!gain.isFinite()||gain.lt(0))fail('夺基输入无效');
    const progress=B.add(p.seize,gain);
    if(progress.lt(100)){s.explorationRewards={...p,seize:String(progress)};return false;}
  ...
  function boundaries(s,rate){if(!B.gt(rate,0))return [];const p=ensure(s),rows=[];
    ...
    if(!s.unlockedAchievements?.seizeFoundation)rows.push({key:'seizeFoundation',pausedReason:null,
      remainingSeconds:B.div(B.max(0,B.sub(100,p.seize)),rate)});
    return rows;
  }
  function settleRetained(s) {
    const p=ensure(s);
    if(s.goldenCoreUnlocked&&belowCap(s)&&B.gte(p.natural.carry,p.natural.remaining))return atomicNatural(s,B.ZERO);
    return B.ZERO;
  }
  ...
  WIS.Cultivation.ExplorationProgress=Object.freeze({fresh,ensure,validate,natural:atomicNatural,settleRetained,seize,view,boundaries,levelWords,capWords,belowCap,
